...
{
    private static Dictionary<Faction, HashSet<TowerType>> _towers;
    public static Dictionary<Faction, HashSet<TowerType>> Towers
    {
        get
        {
            if (_towers == null)
            {
                _towers = new Dictionary<Faction, HashSet<TowerType>>();
                foreach (GameObject building in GameObjectCache.Buildings.Values)
                {
                    ...
                    {
                        _towers.Add(building.GetComponent<Building>().Faction, new HashSet<TowerType>());
                    }
